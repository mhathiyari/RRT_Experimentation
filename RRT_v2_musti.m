function RRT_v1_musti()
    ...
    edge_count = 0;
     for i = 1:iterations
         ...
         q_nearest = v_nearest(q_new.coord,nodes);
         q_new = steer(q_new,q_nearest);
         %if collision_check(p_new,p_nearest,obstacle)
             vert_count = vert_count + 1;
             vertecies(vert_count,:) = q_new.coord;
             edge_count = edge_count + 1;
             edges.x(edge_count,:) = [p_nearest(1),q_new.coord(1)];
             edges.y(edge_count,:) = [p_nearest(2),q_new.coord(2)];
         %end
     end
...
function p_rand = random_point(width,height)
 ...
 function q_nearest = v_nearest(p_rand,nodes)
 q_nearest.cost = sqrt((p_rand(1,1)-nodes(1).coord(1,1))^2+(p_rand(1,2)-nodes(1).coord(1,2))^2);
 q_nearest.coord = nodes(1).coord;
 n = size(nodes);
 for i = 1:n(1)
   v  = nodes(i);
   new_dist = sqrt((p_rand(1,1)-v.coord(1))^2+(p_rand(1,2)-vertecies(2))^2);
   if new_dist< q_nearest.cost
       q_nearest.cost = new_dist;
